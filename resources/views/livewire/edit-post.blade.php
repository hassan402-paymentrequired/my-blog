<div>
    ...
    <form method="POST" wire:submit='updateBlog' class="w-full h-full max-w-3xl p-5 m-auto space-y-3">
        <input wire:model.blur='title' value="{{ $title }}" type="text" class="w-full p-2 text-sm border rounded" placeholder="title" />
        @error('title')
        ...
        <label for="uploadFile1"
            ...
            <template @click="$refs.img.click()" x-if="!obj">
                <div>
                    ...
                    Upload file
                    <input type="file" id="uploadFile1" class="hidden" x-ref="img" wire:model='image' value="{{ $image }}" />
        ...
        <small class="text-xs italic text-red-500">{{ $message }}</small>
    @enderror
        <x-aui::button>Update Blog</x-aui::button>
    </form>
    </>
